#!/usr/bin/env python3
# scripts/render_realistic_explainer.py
#
# Generates a REALISTIC demo explainer MP4 by calling Google Imagen 4 Fast
# for each beat keyframe (clinical medical illustrations) and stitching
# them with Ken-Burns motion + the same agent-activity overlay system as
# the schematic renderer.
#
# Why Imagen 4 instead of the BytePlus pinned image surface:
#   The friendly model IDs pinned in src/lib/seed/models.ts are not
#   provisioned on the current ARK account — POSTs return
#   InvalidEndpointOrModel.NotFound. The .env GEMINI_API_KEY does have
#   access to imagen-4.0-fast-generate-001 and veo-3.x preview, so we
#   route the LIVE realistic-imagery path through Gemini until BytePlus
#   endpoint IDs are provisioned.
#
# This is still a "stand-in" for the deleted-by-PR#11 prewarm pipeline,
# but the visuals are now photorealistic medical illustrations rather
# than vector schematics.
#
# Output:
#   - data/explainers/demo-hip-replacement.mp4
#   - data/replay/demo-hip-replacement/07-seedream/shot_N.png (replaces
#     1×1 stubs with real 1408×768 PNGs from Imagen)
...
# Palette (matches the schematic version + clinical UI tone)
BG = (11, 19, 32)
PANEL_BG = (17, 27, 44)
PANEL_LINE = (41, 56, 81)
ACCENT = (155, 217, 197)
...
# ─── Per-beat prompt overrides ─────────────────────────────────────────────
BEAT_PROMPTS: dict[str, str] = {
    "shot_1": (
        ...
        "skin layer, subcutaneous fat, gluteus maximus muscle. Surgical "
        "drape in pale blue. Soft clinical OR lighting, neutral 5500K, "
        "no harsh shadows. Textbook anatomy style, photo-realistic. "
        "Wide hero shot. No text, no labels, no glyphs, 16:9."
    ),
    "shot_2": (
        "Professional clinical medical illustration of a hip replacement "
        "in progress: joint capsule has been opened, surgeon's gloved "
        "hands are gently dislocating the worn femoral head from the "
        "acetabular socket. Femoral head, neck, and pelvic acetabulum "
        "clearly visible. Light cartilage damage and osteoarthritic "
        "wear pattern on the femoral head surface. Clinical OR lighting, "
        "neutral 5500K. Textbook surgical illustration, photo-realistic. "
        "Hero close-up. No text, no labels, no glyphs, 16:9."
    ),
    "shot_3": (
        "Professional clinical medical illustration of a hip replacement: "
        "an oscillating bone saw cuts the femoral neck at the planned "
        "anatomical angle. The dislocated femoral head sits to the side, "
        "out of the joint. Cut surface clean and angled. Pelvis with "
        "empty acetabulum visible in background. Clinical OR lighting, "
        "neutral 5500K. Textbook surgical illustration, photo-realistic. "
        "Hero close-up. No text, no labels, no glyphs, 16:9."
    ),
    "shot_4": (
        "Professional clinical medical illustration of a hip replacement: "
        "a titanium acetabular cup component being seated into the "
        "prepared acetabular socket using an impactor. The polished "
        "metal cup is visible and partially seated. Pelvis bone shows "
        "reaming pattern. Surgeon's gloved hand holding the impactor "
        "shaft. Clinical OR lighting, neutral 5500K. Textbook surgical "
        "illustration, photo-realistic. Hero close-up. No text, no "
        "labels, no glyphs, 16:9."
    ),
    "shot_5": (
        "Professional clinical medical illustration of a hip replacement: "
        "a titanium femoral stem implanted in the proximal femur shaft, "
        "with a polished cobalt-chrome ball head seated on the trunnion. "
        "The new ball is being trial-reduced into the previously placed "
        "acetabular cup. Surgeon's hands in pale blue gloves. Clinical "
        "OR lighting, neutral 5500K. Textbook surgical illustration, "
        "photo-realistic. Hero close-up. No text, no labels, no glyphs, "
        "16:9."
    ),
    "shot_6": (
        "Professional clinical medical illustration of a hip replacement: "
        "wound closure in progress. Layered sutures visible across the "
        "posterior hip incision — joint capsule, fascia, subcutaneous, "
        "and skin. Surgeon's gloved hands holding a curved suture needle "
        "and forceps. Surgical drapes in pale blue surround the incision. "
        "Clinical OR lighting, neutral 5500K. Textbook surgical "
        "illustration, photo-realistic. Hero close-up. No text, no "
        "labels, no glyphs, 16:9."
    ),
}
...
def draw_agent_rail(im: Image.Image, beat_idx: int) -> None:
    ...
    d.text((x0 + 22, y0 + 18), "AGENT ACTIVITY", fill=SUBTLE, font=f_title)
    ...
    d.text(
        (x0 + 22, inv_y + 22),
        "★ INVARIANT 4: every claim is cited.",
        fill=ACCENT + (255,),
        font=f_inv,
    )
...
def draw_header(im: Image.Image, beat_idx: int, total: int, shot_id: str, citation: str):
    overlay = Image.new("RGBA", im.size, (0, 0, 0, 0))
    d = ImageDraw.Draw(overlay)
    d.rectangle((0, 0, WIDTH, 110), fill=(0, 0, 0, 130))
    f = load_font(22, bold=True)
    chip(d, 56, 46, f"BEAT {beat_idx + 1}/{total}", ACCENT + (255,), PANEL_BG + (220,), f)
    d.text((220, 52), shot_id.upper().replace("_", " "), fill=SUBTLE + (255,), font=f)
    d.text(
        (WIDTH - 600, 52),
        f"CITED →  {citation}",
        fill=SUBTLE + (255,),
...
def draw_narrator_strip(im: Image.Image, narrator: str):
    overlay = Image.new("RGBA", im.size, (0, 0, 0, 0))
    d = ImageDraw.Draw(overlay)
    f = load_font(38)
    ...
    for i, line in enumerate(wrapped):
        d.text((64, y0 + 50 + i * 50), line, fill=TEXT + (255,), font=f)
    im.paste(overlay, (0, 0), overlay)


def draw_phantom_banner(im: Image.Image):
    overlay = Image.new("RGBA", im.size, (0, 0, 0, 0))
    d = ImageDraw.Draw(overlay)
    f = load_font(22)
    s = (
        "Synthetic Phantom   ·   Demo Case   ·   PreOpReel — "
        "imagery: Imagen 4 (live)"
    )
    w, _ = measure(d, s, f)
    d.text(
        ((WIDTH - w) // 2, HEIGHT - 50),
...
# ─── Frame composition ─────────────────────────────────────────────────────
def fit_imagen_to_canvas(png_bytes: bytes) -> Image.Image:
...
def render_beat_keyframe_png(
    *,
    ...
    im = base_image.copy().convert("RGBA")
    vignette = Image.new("RGBA", im.size, (0, 0, 0, 0))
    vd = ImageDraw.Draw(vignette)
    vd.rectangle((1300, 0, WIDTH, HEIGHT), fill=(0, 0, 0, 40))
    im.paste(vignette, (0, 0), vignette)

    draw_agent_rail(im, beat_idx)
    draw_header(im, beat_idx, total, shot_id, citation)
    draw_narrator_strip(im, narrator)
    draw_phantom_banner(im)
...
def main() -> int:
    ...
    try:
        ...
        for i, beat in enumerate(beats):
            ...
            keyframe_png_path = KEYFRAME_DIR / f"{shot_id}.png"
            need_regen = (not keyframe_png_path.exists()) or keyframe_png_path.stat().st_size < 100_000
            prompt = BEAT_PROMPTS.get(shot_id, narrator)
            if need_regen:
                print(f"  beat {i+1}/{len(beats)} {shot_id} — Imagen 4 Fast (~10s)…")
                bytes_ = imagen4_fast(prompt, api_key=api_key)
                keyframe_png_path.write_bytes(bytes_)
            else:
                print(f"  beat {i+1}/{len(beats)} {shot_id} — using cached keyframe")

            base = fit_imagen_to_canvas(keyframe_png_path.read_bytes())
            composed_png = tmp / f"beat_{i+1:02d}_{shot_id}.png"
            render_beat_keyframe_png(
                base_image=base,
                beat_idx=i,
                total=len(beats),
                shot_id=shot_id,
            ...
            clip_mp4 = tmp / f"beat_{i+1:02d}_{shot_id}.mp4"
            direction = "in" if i % 2 == 0 else "out"
            png_to_kenburns_clip(composed_png, clip_mp4, duration, direction=direction)
